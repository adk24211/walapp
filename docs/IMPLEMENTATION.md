...
| | 이전 | 이후 |
|---|---:|---:|
| 목록 (`serviceList`) | 110회 | 11~110회 (perPage 상한에 따라) |
| 상세 (`serviceDetail`) | 110회 | **발행 대상 건수만** (하루 4~5회) |
| 합계 | **220회** | **16~115회** |

**① `perPage` 상향 + 페이지네이션 수정.** `PER_PAGE` 를 100 → 1000 으로 올렸습니다.
odcloud 의 상한을 문서로 확정하지 못해, 크게 요청해 두고 1페이지 응답에서 서버가
실제로 몇 건을 줬는지 보고 그 값으로 낮춥니다.
