...
프로브 결과 상세조회는 **명명된 필드가 아니라 이름/내용 쌍의 섹션 목록**으로 옵니다.

```
servSeCode        070
servSeDetailNm    신청기관연락처목록          ← 섹션 이름
servSeDetailLink  거주지 읍/면/동 주민센터…   ← 섹션 내용
```

즉 `지원대상`·`선정기준`·`지원내용`·`신청방법`이 각각 하나의 섹션으로 오고,
필드명이 아니라 **섹션 이름으로 구분**해야 합니다. `parse_detail()` 이 섹션 이름을
부분 일치로 우리 필드에 매핑합니다(기관마다 '지원대상'/'대상자' 처럼 표기가 흔들려서).

명명 필드형(`tgtrDtlCn` 등)으로 오는 경우도 함께 흡수하도록 두 형태를 모두 파싱합니다.
어느 쪽이 오든 호출부는 구분할 필요가 없습니다.

> 이 구조를 `inspect_api.py` 가 처음엔 잘못 읽었습니다. 반복 요소만 뽑느라
> 최상위 본문 필드를 통째로 놓쳤습니다. 지금은 **최상위 스칼라와 반복 섹션을
> 나눠서 출력**하고, `--raw` 로 응답 원문을 그대로 볼 수도 있습니다.

아직 확인되지 않은 것: 섹션 코드(`010`, `020`…)와 섹션 이름의 정확한 대응표,
그리고 구비서류·신청기한 섹션의 존재 여부. 실 운영에서 채워지지 않으면
해당 제도는 발행되지 않고 반려되므로 **틀린 값이 실릴 위험은 없습니다.**
