...
> `REDESIGN.md` 설계서의 **단계 1~5 구현 완료** + **발행 정책 확정분 반영** (목 데이터 기준).
...
| **6** | `.gitignore` 산출물 차단 해제 · 스케줄 재개 (매일 KST 07:00) | ✅ **완료** |
| 7 | 초기 백필 — 조회수 상위권을 빠르게 채울지 결정 | 첫 자동 실행 관찰 후 |
...
스케줄이 켜졌으므로 매일 KST 07:00 에 자동으로 돕니다. 첫 며칠은 로그를 보세요.

- **`동기화 결과`** — `필드누락` 이 크면 `신청기한` 파싱이나 복지로 `enrich()` 문제
- **`반려 N`** — 복지로는 상세 조회가 하루 100회 제한이라, 후보에 복지로 제도가
  몰리면 일부가 반려됩니다
- **`예정` 건수** — 0건일 수 있습니다. 보조금24 `신청기한` 은 "상시", "예산 소진
  시까지" 같은 자연어가 많아 시작일이 파싱되지 않는 경우가 흔합니다. 0건이면
  시행중 4건만 나가고 예정 발행은 건너뜁니다 (정상 동작)
