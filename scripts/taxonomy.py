...
AUDIENCES: dict[str, dict] = {
    ...
    "jobseeker": {
        ...
        # '직접일자리사업' 은 정부가 일자리를 직접 만들어 참여자를 뽑는 사업이다.
        # 참여자는 곧 구직자인데, 원문이 "직접일자리사업 종합지침에서 정한 사람"
        # 이라고만 쓰면 구직/실업 어느 낱말도 안 나와 태그가 통째로 빠진다.
        # 산림서비스도우미(88,167)가 그래서 어느 대상 허브에도 없었다.
        "keywords": ["구직", "실업", "미취업", "취업 준비", "재취업", "직접일자리사업"],
    },
    "lowincome": {
        "label": "저소득 가구",
        "emoji": "🤝",
        "desc": "소득 기준을 충족하면 받을 수 있는 지원입니다.",
        # ⚠️ '수급권자' 는 '수급자' 로 안 잡힌다 — 사이에 '권' 이 끼어 있다.
        #    의료급여 수급권자는 법으로 정한 소득 기준 대상인데, 의료급여 제도
        #    여섯 건이 이 한 글자 때문에 저소득 허브에서 빠져 있었다.
        #    ('취약계층' 은 여기 넣지 않는다 — 장애인·어르신·한부모까지 포함하는
        #     더 넓은 말이라, 소득 기준으로 읽으면 20건이 잘못 붙는다)
        "keywords": ["기초생활", "차상위", "중위소득", "수급자", "저소득", "수급권자"],
    },
    "business": {
        "label": "소상공인",
        "emoji": "🏪",
    ...
    "student": {
        ...
        # '중고등학교' 계열은 초·중·고를 부르는 아주 흔한 말인데 목록에 없었다.
        # 원문이 "중고등학교에 재학중인 …의 자녀" 라고만 써 놓으면 학생 태그가
        # 통째로 빠진다. 태그를 잃으면 받을 수 있는 사람이 그 제도를 못 찾는다 —
        # 잘못 붙는 것보다 나쁘다(check_audience.py 머리 주석).
        "keywords": ["고등학생", "중학생", "초등학생", "고교", "고졸",
                     "중고등학교", "중·고등학교", "초중고", "초·중·고"],
    },
    # ── 사업주 ──
    #
...
NOT_AUDIENCE_PHRASES: dict[str, tuple[str, ...]] = {
    ...
    # 법령·기관 이름 안의 '노인'
    #
    # '노인요양시설' 을 늦게 알아챘다. 전기 요금 복지할인(156,440)이 "…감액대상에서
    # 제외합니다 · 노인복지법 시행규칙 제18조 … 라목에 해당하는 **노인요양시설**"
    # 한 자리 때문에 어르신 허브에 남아 있었다. 앞의 '노인복지법' 은 이미 걸러지고
    # 있었는데 뒤의 시설 이름이 안 걸러졌다.
    # 승강기검사수수료 감면(노인복지시설)도 함께 빠진다 — 그쪽은 받는 쪽이
    # '시설에 설치된 승강기' 라, 어르신이 신청할 수 있는 제도가 아니다.
    #
    # '준고령' 은 뜻이 다르다. 이 사이트의 어르신 축은 화면에 **만 65세 이상**
    # 이라고 적혀 있는데, 준고령자는 만 50~64세다. 통합문화이용권(64,002)이
    # "준고령기(60-64세) 1만원 추가 지원" 한 줄로, 준고령자 직업선호도검사는
    # "만 50세 이상 준고령자" 로 어르신에 들어와 있었다.
    "senior": ("노인복지법", "노인복지주택", "노인장기요양기관", "노인복지시설",
               "노인요양시설", "준고령"),
    # '인지기능장애 검사' 는 검진 **항목 이름**이지 대상이 아니다. 의료급여
    # 검진 제도 두 건이 이것 하나로 장애인 허브에 올라가 있었다.
    "disabled": ("장애인복지법", "인지기능장애"),
    ...
    # '사업주는 …하여야 함' 은 사업주의 **의무**를 적은 것이지 사업주가 받는다는
    # 뜻이 아니다. 육아휴직급여(542,419)의 사업주 언급이 전부 그 모양이었다 —
    # "사업주는 1년간 육아휴직을 허용하여야 함", "(무급: 사업주는 임금 지급의무X)".
    # 급여는 근로자가 고용센터에서 받는다.
