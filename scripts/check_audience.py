...
CASES: list[tuple[str, str, tuple[str, ...], tuple[str, ...]]] = [
    ...
    # ── 놓치고 있던 말들 (태그가 아예 안 붙던 경우) ──
    ("'중고등학교' 도 초·중·고를 부르는 말이다",
     "발전소 주변지역 장학금 지원 인천발전본부 반경 5km 이내 중고등학교에 재학중인 "
     "인천시 실거주 1년 이상인 자의 자녀 및 본인",
     (), ("student",)),
    ("'수급권자' 는 '수급자' 로 안 잡힌다 — 사이에 '권' 이 끼어 있다",
     "의료급여수급권자 일반건강검진비 지원 의료급여 수급권자 중 만 19세 이상 세대원",
     (), ("lowincome",)),
    # '취약계층' 을 소득 기준으로 읽으면 안 된다 — 장애인·어르신·한부모까지
    # 포함하는 더 넓은 말이라 20건이 잘못 붙는다.
    ("'취약계층' 만으로는 저소득 가구가 아니다",
     "기획공연 및 전시 관람료 할인 취약계층 및 지역주민 대상 문화행사 관람료 감면",
     ("lowincome",), ()),

    # ── 원문만 보고 독립 판정한 감사(조회수 상위 20건)가 잡아낸 것들 ──
    ("제외 목록 안의 '노인요양시설'",
     "전기 요금 복지할인 다만, 다음 법령에 해당하는 사회복지시설은 감액대상에서 제외합니다. "
     "노인복지법 시행규칙 제18조 제1항 제1호 라목에 해당하는 노인요양시설",
     ("senior",), ()),
    ("'준고령'(만 50~64세)은 어르신 축(만 65세 이상)이 아니다",
     "준고령자 직업선호도검사 서비스 만 50세 이상 준고령자",
     ("senior",), ()),
    ("'인지기능장애 검사' 는 검진 항목 이름이지 대상이 아니다",
     "의료급여수급권자 일반건강검진비 지원 구강검진, 심뇌혈관질환 위험평가, 인지기능장애 검사, B형 간염검사",
     ("disabled",), ()),
    ("'사업주는 …하여야 함' 은 의무이지 수혜가 아니다",
     "육아휴직급여 자녀를 양육하기 위하여 육아휴직을 신청하는 경우 사업주는 1년간 육아휴직을 "
     "허용하여야 함(무급: 사업주는 임금 지급의무X)",
     ("employer",), ("parent",)),
    ("정부가 직접 만드는 일자리의 참여자는 구직자다",
     "산림서비스도우미(산림복지일자리) 신청일 기준으로 만 18세 이상인 자로서 "
     "사업별 지침에서 정한 사람(직접일자리사업 종합지침)",
     (), ("jobseeker",)),

    # ⚠️ 대상 분류용 blob 은 줄바꿈으로 이어야 한다. 공백으로 이으면
    #    strip_exclusion_sections 가 '지원 제외 대상' 다음 필드까지 삼킨다.
    #    아래는 지원대상 마지막 줄과 선정기준 첫 줄을 줄바꿈으로 이어 둔 모양이고,
    #    공백으로 이으면 선정기준이 통째로 사라져 이 케이스가 깨진다.
    ("'지원 제외 대상' 이 다음 필드를 삼키지 않는다",
     "국민내일배움카드\n○ 국민 누구나\n\n○ 지원 제외 대상\n"
     "① 공무원, 사립학교 교직원\n⑥ 만 75세 이상자 등\n"
     "○ 직업경력, 직업 능력 수준, 취업희망 분야, 직업훈련 경험 등 "
     "직업훈련의 필요성에 관하여 훈련상담을 실시\n"
     "○ 만 19세 이상 청년 우대",
     (), ("youth",)),
]
