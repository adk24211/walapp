...
def build_prompt(record: ProgramRecord) -> str:
    ...
    rules = textwrap.dedent(r"""
        ...
        ★ 단서 규칙 (틀리는 방식이 가장 일정한 자리)
        원문은 조건을 '단', '다만', '*', 괄호로 좁혀 놓습니다. 그 한정을 지우고
        일반화하면 **없는 자격을 만들어 내는 것**과 같습니다. 실제로 이렇게 틀렸습니다.

          원문 "단, 청년외의 경우 보증료의 90%를 지원"
          틀림 "보증료의 90%까지 지원됩니다"        ← 청년은 전액인데 90%로 깎았습니다
          원문 "한부모가정(취업하였거나, 비취업인 경우 장애인 또는 다자녀 양육)"
          틀림 "한부모가정(취업 여부와 관계없이)"   ← 조건을 정반대로 뒤집었습니다
          원문 "정부지원 최고한도 10만 원 적립 후 월 50만 원 내에서 추가 적립 가능"
          틀림 "추가 적립은 10만 원을 초과할 수 없습니다"  ← 상한을 뒤집었습니다
          원문 "단축을 시작한 날 이후 1개월부터 끝난 날 이후 12개월 이내"
          틀림 "종료 후 1개월부터 12개월 이내"      ← 신청 시작점을 옮겼습니다
          원문 "(신규지원 시 예외대상) 소재·부품·장비 강소기업 100 …"
          틀림 "… 등은 제외됩니다"                 ← 지원 가능한 예외를 제외로 바꿨습니다
          원문 "자가 사업장 확보 자금은 기업당 3년 이내 1회로 한정"
          틀림 "시설자금은 기업당 3년 이내 1회"     ← 하위 항목의 제한을 상위로 넓혔습니다

        - 단서가 붙은 사실은 **단서와 함께** 쓰거나, 자신이 없으면 아예 쓰지 마세요.
        - '누구에게 적용되는가'(청년/청년외), '언제부터인가'(시작일/종료일),
          '무엇에 걸리는가'(전체/하위 항목)를 원문 그대로 유지하세요.
        - 원문이 말하지 않은 것을 단정하지 마세요. "만기 시에만", "전액에 동일하게"
          처럼 원문에 없는 배타·확대 표현을 붙이지 않습니다.

        ★ 옮겨 적기 금지
        원문 문장에 조사와 어미만 바꿔 붙인 항목은 쓰지 마세요. 원문은 같은
        페이지에 '원문 펼치기' 로 이미 실립니다. 같은 문장을 두 번 읽게 할 뿐입니다.
          원문 "○ 경기도내 거주 장애인 및 70세 이상 본인 또는 동반 1인"
          틀림 "경기도 내 거주 장애인 및 70세 이상 본인 또는 동반 1인입니다."
        풀어 쓸 것이 없으면 그 항목을 만들지 마세요. 적게 쓰는 편이 낫습니다.

        ★ 같은 말 두 곳에 쓰지 않기
        주의 안내(note)와 자주 묻는 질문(faq)과 자격(eligibility)은 서로 다른
        내용이어야 합니다. 한 사실을 세 곳에 나눠 적으면 분량만 늘고 읽는 사람은
        같은 문장을 세 번 봅니다. 자동 검사가 뒤엣것을 버립니다.
