...
def _build_prompt(
    ...
    rules = textwrap.dedent(r"""
        ...
        ★ 반복·분량 늘리기 금지(매우 중요)
        - 같은 문장이나 같은 내용을 반복해 분량을 늘리지 않습니다. 한 번 말한 내용을 다시 풀어 쓰지 않습니다.
        - 본문 자료가 빈약하면 억지로 길게 쓰지 말고, 확인된 사실만 간결하게 전달합니다(짧아도 됩니다).
        - 의미 없는 일반론('중요한 역할을 합니다', '도움이 됩니다')만 반복하지 않습니다. 구체적 사실로 채웁니다.
        - steps는 2~5개로, 본문에서 다룰 내용이 충분한 만큼만 만듭니다. 내용이 있으면 각 body 4문장 이상.
