...
def _build_prompt(
    ...
    rules = textwrap.dedent(r"""
        ...
        - 같은 문장이나 같은 뜻의 문장을 두 번 이상 쓰지 않습니다.
          (나쁜 예: "지원하는 정책입니다. 이 정책은 지원합니다. 지원하는 정책입니다" — 이렇게 쓰면 안 됩니다.)
        - 본문 자료가 제목 수준으로 빈약하면, 억지로 늘리지 말고 확인 가능한 사실만 1~3문장으로 간결히 씁니다. 짧아도 괜찮습니다.
        - 자료에 근거가 없는 수치·조건(가입 기간, 금액 등)을 추측해서 적지 않습니다.
        - 의미 없는 일반론('중요한 역할을 합니다', '도움이 됩니다')으로 채우지 않습니다.
        - steps 개수는 '자료로 충실히 쓸 수 있는 만큼'만 만듭니다(2~5개). 내용이 부족한 항목은 아예 넣지 않습니다.
        - 각 step은 '서로 다른 내용'을 다뤄야 합니다. 두 step이 같은 내용을 다른 말로 반복하면 안 됩니다. 쓸 거리가 1~2가지뿐이면 step도 1~2개만 만듭니다.
        ...
        - quote: 글의 핵심 통찰을 압축한 한 문장. lead나 본문 문장을 그대로 반복하지 말고 새로 씁니다. 없으면 생략 가능.
