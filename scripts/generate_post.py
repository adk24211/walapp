...
def _build_prompt(
    ...
    if category == "policy":
        instruction = textwrap.dedent("""
            ...
            - 가장 중요한 정책 1~2개를 선정해 신청 자격, 기간, 지원 금액 등 사실을 구체적인 수치와 함께 보도하세요
            - 주관적 권유 대신 사실 전달 중심으로 서술하되, 신청 방법은 문단 말미에 객관적으로 명시하세요
        """).strip()
        ...
        instruction = textwrap.dedent(f"""
            ...
            - 첫 문단은 이번 주 채용 시장의 흐름을 요약하는 리드 문장으로 시작하세요
            - 기술 스택 트렌드를 언급 빈도 등 수치 근거와 함께 보도하세요
            - 주목할 만한 공고나 동향 3~5개를 사실 중심으로 소개하세요
            - 마지막 문단에서 구직 시장 전망을 객관적으로 짚어주세요
            {stack_info}
        """).strip()
        ...
        instruction = textwrap.dedent("""
            ...
            - 각 뉴스는 핵심 사실을 먼저 전달하고, 그 의미와 파급 효과를 한 문장으로 분석하세요
            - 영어 뉴스는 한국어 기사체로 정확하게 옮기세요
            - 글로벌 동향이 국내 개발 생태계에 미치는 영향을 사실에 근거해 서술하세요
        """).strip()
