...
VERIFIED_BY_HAND = {
    ...
    # 2026-08-25 adk24211 — 아래 넷은 브라우저 주소창의 사이트 정보 팝업에서
    # "이 연결은 안전합니다" 를 직접 확인했다. 각각 기관 메인으로 리다이렉트되며
    # 스킴은 https 로 유지된다.
    #
    # 러너에서는 넷 다 HEAD·GET 이 막힌다(해외 데이터센터 IP). 그래서 매일 도는
    # upgrade_urls.py 가 자동으로는 못 올린다.
    #
    # ⚠️ 확인 과정에서 헷갈렸던 것 — 처음엔 "자물쇠가 안 뜬다" 는 보고였다.
    #    크롬 117 부터 자물쇠 아이콘이 슬라이더 모양으로 바뀌어서 그렇게 보인
    #    것이고, 팝업을 열면 안전하다고 나온다. 인증서 문제가 진짜 있으면
    #    주소창에 '주의 요함'·'안전하지 않음' 같은 **글자**가 함께 뜬다.
    #    다음에 이 목록을 늘릴 때도 아이콘 모양이 아니라 그 글자로 판단할 것.
    "myhome.go.kr",      # 마이홈포털 — 행복주택·국민임대·영구임대 3건
    "edu.kinfa.or.kr",   # 서민금융진흥원 금융교육
    "kinfa.or.kr",       # 서민금융진흥원
    "www.kdhc.co.kr",    # 한국지역난방공사 — https://kdhc.co.kr 로 리다이렉트
}
